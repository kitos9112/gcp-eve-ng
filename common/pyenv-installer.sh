...
#[[ -f user-vars.sh ]] && . user-vars.sh || echo "No users var file found"
[ -n "$PYENV_DEBUG" ] && set -x
...
if ! command -v pyenv 1>/dev/null; then
  ...
  if [[ $POPULATE_SHELL_PROFILE && $SHELL_PROFILE_FILE != "" ]] ; then
    echo "Populating your shell"
    case "$SHELL" in
    fish )
      echo "set -x PATH \"${PYENV_ROOT}/bin\" \$PATH" >> $SHELL_PROFILE_FILE
      echo 'status --is-interactive; and . (pyenv init -|psub)' >> $SHELL_PROFILE_FILE
      echo 'status --is-interactive; and . (pyenv virtualenv-init -|psub)' >> $SHELL_PROFILE_FILE
      ;;
    * )
      echo "export PATH=\"${PYENV_ROOT}/bin:\$PATH\"" >> $SHELL_PROFILE_FILE
      echo "eval \"\$(pyenv init -)\"" >> $SHELL_PROFILE_FILE
      echo "eval \"\$(pyenv virtualenv-init -)\"" >> $SHELL_PROFILE_FILE
      ;;
    esac
  else
  {
    echo "# Load pyenv automatically by adding"
    echo "# the following to your SHELL profile manually:"
    echo
    case "$shell" in
    fish )
